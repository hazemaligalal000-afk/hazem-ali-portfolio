...
(function () {
    ...
    async function loadPage(page) {
        ...
        switch (page) {
            ...
            case 'webhooks': title.innerText = 'Webhooks'; renderWebhooks(el); break;
            ...
            case 'email-campaigns': title.innerText = 'Email Campaigns'; renderEmailCampaigns(el); break;
    ...
    async function renderCaseStudies(el) {
        try {
            ...
            el.innerHTML = `
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title">Portfolio & Case Studies</h3>
                        <button class="btn-sm btn-sm-primary" onclick="window.openCaseStudyModal()">Add Case Study</button>
                    </div>
                    <div class="table-wrap">
                        <table>
                            <thead>
                            ...
                            <tbody>
                                ${data.map(c => `
                                    <tr>
                                        ...
                                        <td><strong>${c.title_ar}</strong></td>
                                        <td><span class="badge ${c.industry ? 'badge-primary' : 'badge-default'}">${c.industry || 'General'}</span></td>
                                        <td>
                                        ...
                                        <td>
                                            <button class="btn-sm btn-sm-outline" onclick='window.editCaseStudy(${JSON.stringify(c).replace(/'/g, "&apos;")})'>Edit</button>
                                            <button class="btn-sm btn-sm-danger" onclick="window.deleteCaseStudy('${c.uuid}')">Delete</button>
                                        </td>
                                    </tr>
                                `).join('') || '<tr><td colspan="5" class="text-center">No case studies found.</td></tr>'}
                            </tbody>
                        </table>
                    </div>
                </div>
                ...
                <div class="modal-overlay" id="case-study-modal">
                    <div class="modal-content" style="max-width: 600px;">
                        <div class="modal-header">
                            <h3 id="cs-modal-title">Add Case Study</h3>
                            <button class="modal-close" onclick="document.getElementById('case-study-modal').classList.remove('active')">&times;</button>
                        </div>
                        <form id="case-study-form" class="modal-body">
                            <input type="hidden" id="cs-uuid">
                            <div class="admin-form-group">
                            ...
                            <div class="admin-form-group">
                                ...
                                <textarea id="cs-desc" rows="3" required></textarea>
                            </div>
                            <div style="display:flex; gap:15px;">
                                <div class="admin-form-group" style="flex:1;"><label>Industry</label><input type="text" id="cs-industry"></div>
                                <div class="admin-form-group" style="flex:1;"><label>Display Order</label><input type="number" id="cs-order" value="0"></div>
                            </div>
                            <div class="admin-form-group">
                                <label>Main KPI (e.g. +300% ROAS)</label>
                                <div style="display:flex; gap:10px;">
                                    <input type="text" id="cs-stat1-val" placeholder="Value (e.g. 300%)" style="flex:1;">
                                    <input type="text" id="cs-stat1-label" placeholder="Label (e.g. ROAS)" style="flex:1;">
                                </div>
                            </div>
                            <div class="admin-form-group">
                                <label>Image URL (Optional)</label>
                                <input type="text" id="cs-img">
                            </div>
                            <div class="admin-form-group">
                                <label style="display:flex; align-items:center; gap:8px;">
                                    <input type="checkbox" id="cs-active" checked> Active (Visible on site)
                                </label>
                            </div>
                            <button type="submit" class="btn-primary" style="width:100%; margin-top:15px;">Save Case Study</button>
                        </form>
                    </div>
                </div>
            `;
            ...
            window.openCaseStudyModal = () => {
                ...
                document.getElementById('cs-uuid').value = '';
                document.getElementById('cs-modal-title').innerText = 'Add Case Study';
                document.getElementById('case-study-modal').classList.add('active');
            };

            window.editCaseStudy = (c) => {
                document.getElementById('cs-uuid').value = c.uuid;
                document.getElementById('cs-title').value = c.title_ar;
                document.getElementById('cs-desc').value = c.description_ar;
                document.getElementById('cs-industry').value = c.industry || '';
                document.getElementById('cs-order').value = c.display_order || 0;
                document.getElementById('cs-stat1-val').value = c.stat_1_value || '';
                document.getElementById('cs-stat1-label').value = c.stat_1_label || '';
                document.getElementById('cs-img').value = c.image_url || '';
                document.getElementById('cs-active').checked = c.is_active === 1;
                document.getElementById('cs-modal-title').innerText = 'Edit Case Study';
                document.getElementById('case-study-modal').classList.add('active');
            };

            window.deleteCaseStudy = async (uuid) => {
            ...
            document.getElementById('case-study-form').onsubmit = async (e) => {
                e.preventDefault();
                const uuid = document.getElementById('cs-uuid').value;
                const payload = {
                    title_ar: document.getElementById('cs-title').value,
                    description_ar: document.getElementById('cs-desc').value,
                    industry: document.getElementById('cs-industry').value,
                    display_order: parseInt(document.getElementById('cs-order').value),
                    stat_1_value: document.getElementById('cs-stat1-val').value,
                    stat_1_label: document.getElementById('cs-stat1-label').value,
                    image_url: document.getElementById('cs-img').value,
                    is_active: document.getElementById('cs-active').checked ? 1 : 0
                };
                
                await api(uuid ? \`/admin/case-studies/\${uuid}\` : '/admin/case-studies', {
            ...
            el.innerHTML = '<div class="empty-state">Error loading case studies.</div>';
        }
    }
    ...
    async function renderWebhooks(el) {
        try {
            ...
            el.innerHTML = `
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title">Webhook Integrations (CRM / Zapier)</h3>
                        <button class="btn-sm btn-sm-primary" onclick="window.openWebhookModal()">Add Webhook</button>
                    </div>
                    <div class="card-body-padded" style="border-bottom: 1px solid var(--border-color); padding-bottom: 20px;">
                        <p style="color: var(--text-gray); font-size: 0.9rem;">Webhooks allow you to automatically send new lead data to external systems like Zapier, Make.com, HubSpot, or GoHighLevel.</p>
                    </div>
                    <div class="table-wrap">
                        <table>
                            <thead>
                            ...
                            <tbody>
                                ${data.map(w => `
                                    <tr>
                                        <td><code style="background:#f1f3f5; padding:4px 8px; border-radius:4px;">${w.url}</code></td>
                                        <td><span class="badge badge-primary">${w.event_type}</span></td>
                                        <td>
                                        ...
                                        <td>
                                            <button class="btn-sm btn-sm-outline" onclick='window.editWebhook(${JSON.stringify(w).replace(/'/g, "&apos;")})'>Edit</button>
                                            <button class="btn-sm btn-sm-danger" onclick="window.deleteWebhook(${w.id})">Delete</button>
                                        </td>
                                    </tr>
                                `).join('') || '<tr><td colspan="4" class="text-center">No active webhooks configured.</td></tr>'}
                            </tbody>
                        </table>
                    </div>
                </div>
                ...
                <div class="modal-overlay" id="webhook-modal">
                    <div class="modal-content" style="max-width: 500px;">
                        <div class="modal-header">
                            ...
                            <button class="modal-close" onclick="document.getElementById('webhook-modal').classList.remove('active')">&times;</button>
                        </div>
                        <form id="webhook-form" class="modal-body">
                            <input type="hidden" id="wh-id">
                            <div class="admin-form-group">
                                <label>Endpoint URL *</label>
                                <input type="url" id="wh-url" placeholder="https://hooks.zapier.com/..." required>
                            </div>
                            <div class="admin-form-group">
                                <label>Trigger Event</label>
                                <select id="wh-event">
                                    <option value="new_lead">New Lead Submission</option>
                                </select>
                            </div>
                            <div class="admin-form-group">
                                <label style="display:flex; align-items:center; gap:8px;">
                                    <input type="checkbox" id="wh-active" checked> Active
                                </label>
                            </div>
                            <button type="submit" class="btn-primary" style="width:100%; margin-top:15px;">Save Webhook</button>
                        </form>
                    </div>
                </div>
            `;
            ...
            window.openWebhookModal = () => {
                ...
                document.getElementById('wh-modal-title').innerText = 'Add Webhook';
                document.getElementById('webhook-modal').classList.add('active');
            };

            window.editWebhook = (w) => {
                document.getElementById('wh-id').value = w.id;
                document.getElementById('wh-url').value = w.url;
                document.getElementById('wh-event').value = w.event_type;
                document.getElementById('wh-active').checked = w.is_active === 1;
                document.getElementById('wh-modal-title').innerText = 'Edit Webhook';
                document.getElementById('webhook-modal').classList.add('active');
            };
            ...
            document.getElementById('webhook-form').onsubmit = async (e) => {
                ...
                await api(id ? \`/admin/webhooks/\${id}\` : '/admin/webhooks', {
                    method: id ? 'PUT' : 'POST',
                    body: JSON.stringify(payload)
                });
            ...
            el.innerHTML = '<div class="empty-state">Error loading webhooks.</div>';
        }
    }
    ...
    async function renderTestimonials(el) {
        try {
            ...
            el.innerHTML = `
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title">Client Testimonials</h3>
                        <button class="btn-sm btn-sm-primary" onclick="window.openTestimonialModal()">Add Testimonial</button>
                    </div>
                    <div class="table-wrap">
                        <table>
                            <thead>
                            ...
                            <tbody>
                                ${data.map(t => `
                                    <tr>
                                        <td><strong>${t.client_name}</strong><br><small style="color:var(--text-gray)">${t.client_role || ''}</small></td>
                                        <td>${t.client_company || '-'}</td>
                                        <td>${'⭐'.repeat(t.rating)}</td>
                                        <td>
                                            <span class="badge ${t.is_active ? 'badge-success' : 'badge-default'}">
                                                ${t.is_active ? 'Published' : 'Hidden'}
                                            </span>
                                        </td>
                                        <td>
                                            <button class="btn-sm btn-sm-outline" onclick='window.editTestimonial(${JSON.stringify(t).replace(/'/g, "&apos;")})'>Edit</button>
                                            <button class="btn-sm btn-sm-danger" onclick="window.deleteTestimonial('${t.uuid}')">Delete</button>
                                        </td>
                                    </tr>
                                `).join('') || '<tr><td colspan="5" class="text-center">No testimonials found.</td></tr>'}
                            </tbody>
                        </table>
                    </div>
                </div>
                ...
                <div class="modal-overlay" id="testimonial-modal">
                    <div class="modal-content" style="max-width: 500px;">
                        <div class="modal-header">
                            <h3 id="test-modal-title">Add Testimonial</h3>
                            <button class="modal-close" onclick="document.getElementById('testimonial-modal').classList.remove('active')">&times;</button>
                        </div>
                        <form id="testimonial-form" class="modal-body">
                            <input type="hidden" id="test-uuid">
                            <div class="admin-form-group"><label>Client Name *</label><input type="text" id="test-name" required></div>
                            <div style="display:flex; gap:15px;">
                                <div class="admin-form-group" style="flex:1;"><label>Role</label><input type="text" id="test-role"></div>
                                <div class="admin-form-group" style="flex:1;"><label>Company</label><input type="text" id="test-company"></div>
                            </div>
                            <div class="admin-form-group"><label>Review Content (Arabic) *</label><textarea id="test-content" rows="4" required></textarea></div>
                            <div style="display:flex; gap:15px;">
                                <div class="admin-form-group" style="flex:1;"><label>Rating (1-5)</label><input type="number" id="test-rating" min="1" max="5" value="5"></div>
                                <div class="admin-form-group" style="flex:1;"><label>Order</label><input type="number" id="test-order" value="0"></div>
                            </div>
                            <div class="admin-form-group"><label>Avatar Image URL</label><input type="text" id="test-img"></div>
                            <div class="admin-form-group"><label style="display:flex; align-items:center; gap:8px;"><input type="checkbox" id="test-active" checked> Published</label></div>
                            <button type="submit" class="btn-primary" style="width:100%; margin-top:15px;">Save Testimonial</button>
                        </form>
                    </div>
                </div>
            `;

            window.openTestimonialModal = () => { document.getElementById('testimonial-form').reset(); document.getElementById('test-uuid').value = ''; document.getElementById('test-modal-title').innerText = 'Add Testimonial'; document.getElementById('testimonial-modal').classList.add('active'); };
            window.editTestimonial = (t) => {
                document.getElementById('test-uuid').value = t.uuid; document.getElementById('test-name').value = t.client_name; document.getElementById('test-role').value = t.client_role || '';
                document.getElementById('test-company').value = t.client_company || ''; document.getElementById('test-content').value = t.content_ar || ''; document.getElementById('test-rating').value = t.rating || 5;
                document.getElementById('test-order').value = t.display_order || 0; document.getElementById('test-img').value = t.image_url || ''; document.getElementById('test-active').checked = t.is_active === 1;
                document.getElementById('test-modal-title').innerText = 'Edit Testimonial'; document.getElementById('testimonial-modal').classList.add('active');
            };
            window.deleteTestimonial = async (uuid) => { if (confirm('Delete testimonial?')) { await api(\`/admin/testimonials/\${uuid}\`, { method: 'DELETE' }); loadPage('testimonials'); } };
            document.getElementById('testimonial-form').onsubmit = async (e) => {
                e.preventDefault();
                const uuid = document.getElementById('test-uuid').value;
                const payload = { client_name: document.getElementById('test-name').value, client_role: document.getElementById('test-role').value, client_company: document.getElementById('test-company').value, content_ar: document.getElementById('test-content').value, rating: parseInt(document.getElementById('test-rating').value), display_order: parseInt(document.getElementById('test-order').value), image_url: document.getElementById('test-img').value, is_active: document.getElementById('test-active').checked ? 1 : 0 };
                await api(uuid ? \`/admin/testimonials/\${uuid}\` : '/admin/testimonials', { method: uuid ? 'PUT' : 'POST', body: JSON.stringify(payload) });
                document.getElementById('testimonial-modal').classList.remove('active'); loadPage('testimonials');
            };
        } catch (e) { el.innerHTML = '<div class="empty-state">Error loading testimonials.</div>'; }
    }
            ...
            el.innerHTML = `
                <div class="card">
                    <div class="card-header">
                        ...
                        <button class="btn-sm btn-sm-primary" onclick="window.openSubscriberModal()">Add Manually</button>
                    </div>
                    <div class="table-wrap">
                        <table>
                            <thead><tr><th>Email</th><th>Source</th><th>Status</th><th>Subscribed At</th><th>Actions</th></tr></thead>
                            <tbody>
                                ${data.map(s => `
                                    <tr>
                                        <td><strong>${s.email}</strong></td>
                                        <td><span class="badge badge-default">${s.source}</span></td>
                                        <td><span class="badge ${s.is_active ? 'badge-success' : 'badge-danger'}">${s.is_active ? 'Subscribed' : 'Unsubscribed'}</span></td>
                                        <td>${new Date(s.created_at).toLocaleDateString()}</td>
                                        <td>
                                            <button class="btn-sm btn-sm-danger" onclick="window.deleteSubscriber(${s.id})">Delete</button>
                                        </td>
                                    </tr>
                                `).join('') || '<tr><td colspan="5" class="text-center">No subscribers yet.</td></tr>'}
                            </tbody>
                        </table>
                    </div>
                </div>
                ...
                <div class="modal-overlay" id="subscriber-modal">
                    <div class="modal-content" style="max-width: 400px;">
                        <div class="modal-header"><h3>Add Subscriber</h3><button class="modal-close" onclick="document.getElementById('subscriber-modal').classList.remove('active')">&times;</button></div>
                        <form id="subscriber-form" class="modal-body">
                            <div class="admin-form-group"><label>Email *</label><input type="email" id="sub-email" required></div>
                            <button type="submit" class="btn-primary" style="width:100%; margin-top:15px;">Add Subscriber</button>
                        </form>
                    </div>
                </div>
            `;
            window.openSubscriberModal = () => { document.getElementById('subscriber-form').reset(); document.getElementById('subscriber-modal').classList.add('active'); };
            window.deleteSubscriber = async (id) => { if (confirm('Delete subscriber?')) { await api(\`/admin/subscribers/\${id}\`, { method: 'DELETE' }); loadPage('subscribers'); } };
            document.getElementById('subscriber-form').onsubmit = async (e) => {
                e.preventDefault();
                try {
                    await api('/admin/subscribers', { method: 'POST', body: JSON.stringify({ email: document.getElementById('sub-email').value, source: 'admin_manual' }) });
                    document.getElementById('subscriber-modal').classList.remove('active'); loadPage('subscribers');
                } catch (err) { alert(err.message); }
            };
        } catch (e) { el.innerHTML = '<div class="empty-state">Error loading subscribers.</div>'; }
    }

    // ============================================
    // EMAIL CAMPAIGNS MANAGER
    // ============================================
    async function renderEmailCampaigns(el) {
        try {
            const { data } = await api('/admin/email-campaigns');
            el.innerHTML = `
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title">Bulk Email Campaigns</h3>
                        <button class="btn-sm btn-sm-primary" onclick="window.openCampaignModal()">Draft New Campaign</button>
                    </div>
                    <div class="table-wrap">
                        <table>
                            <thead><tr><th>Subject</th><th>Status</th><th>Sent At</th><th>Actions</th></tr></thead>
                            <tbody>
                                ${data.map(c => `
                                    <tr>
                                        <td><strong>${c.subject}</strong></td>
                                        <td><span class="badge ${c.status === 'sent' ? 'badge-success' : 'badge-warning'}">${c.status.toUpperCase()}</span></td>
                                        <td>${c.sent_at ? new Date(c.sent_at).toLocaleString() : '-'}</td>
                                        <td>
                                            ${c.status === 'draft' ? `<button class="btn-sm btn-sm-primary" onclick="window.sendCampaign(${c.id})">Send Now</button>` : ''}
                                            <button class="btn-sm btn-sm-outline" onclick='window.editCampaign(${JSON.stringify(c).replace(/'/g, "&apos;")})'>Edit</button>
                                            <button class="btn-sm btn-sm-danger" onclick="window.deleteCampaign(${c.id})">Delete</button>
                                        </td>
                                    </tr>
                                `).join('') || '<tr><td colspan="4" class="text-center">No campaigns yet.</td></tr>'}
                            </tbody>
                        </table>
                    </div>
                </div>
                ...
                <div class="modal-overlay" id="campaign-modal">
                    <div class="modal-content" style="max-width: 700px;">
                        <div class="modal-header">
                            <h3 id="camp-modal-title">Draft Campaign</h3>
                            <button class="modal-close" onclick="document.getElementById('campaign-modal').classList.remove('active')">&times;</button>
                        </div>
                        <form id="campaign-form" class="modal-body">
                            <input type="hidden" id="camp-id">
                            <div class="admin-form-group"><label>Email Subject *</label><input type="text" id="camp-subject" required></div>
                            <div class="admin-form-group">
                                <label>Email Body (HTML format) *</label>
                                <textarea id="camp-body" rows="10" required placeholder="<h1>Hello!</h1><p>This is a newsletter...</p>"></textarea>
                            </div>
                            <button type="submit" class="btn-primary" style="width:100%; margin-top:15px;">Save Draft</button>
                        </form>
                    </div>
                </div>
            `;
            window.openCampaignModal = () => { document.getElementById('campaign-form').reset(); document.getElementById('camp-id').value = ''; document.getElementById('camp-modal-title').innerText = 'Draft Campaign'; document.getElementById('campaign-modal').classList.add('active'); };
            window.editCampaign = (c) => { document.getElementById('camp-id').value = c.id; document.getElementById('camp-subject').value = c.subject; document.getElementById('camp-body').value = c.body_html; document.getElementById('camp-modal-title').innerText = 'Edit Campaign Draft'; document.getElementById('campaign-modal').classList.add('active'); };
            window.deleteCampaign = async (id) => { if (confirm('Delete campaign?')) { await api(\`/admin/email-campaigns/\${id}\`, { method: 'DELETE' }); loadPage('email-campaigns'); } };
            window.sendCampaign = async (id) => {
                if (confirm('Are you sure you want to SEND this email to ALL active subscribers?')) {
                    try {
                        const res = await api(\`/admin/email-campaigns/\${id}/send\`, { method: 'POST' });
                        showToast(res.message);
                        loadPage('email-campaigns');
                    } catch (err) { alert(err.message); }
                }
            };
            document.getElementById('campaign-form').onsubmit = async (e) => {
                ...
                const payload = { subject: document.getElementById('camp-subject').value, body_html: document.getElementById('camp-body').value };
                await api(id ? \`/admin/email-campaigns/\${id}\` : '/admin/email-campaigns', { method: id ? 'PUT' : 'POST', body: JSON.stringify(payload) });
                document.getElementById('campaign-modal').classList.remove('active'); loadPage('email-campaigns');
            };
        } catch (e) { el.innerHTML = '<div class="empty-state">Error loading campaigns.</div>'; }
    }
